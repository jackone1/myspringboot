...
public class StringUtil {
	...
	public static String subEndsWith(String str, String suffix) {
	...
	/**
	 * 模糊查询处理 功能：模糊查询时，处理特殊符号：如“%”，“_”,"/"等 <br>
	 * @param str 字符
	 * @return 处理后的值
	 */
	public static String processSymbol(String str) {
		if (!isEmpty(str)) {
			...
			// 如果第一个字符是“/”, 上述处理过去变成“//”,这时要特殊处理变成“///” 之后，在mysSql中才可以查询
			if (str.startsWith("//")) {
				str = "/" + str;
			}
			// 返回处理结果时在开始和结尾加上“%”（模糊查询）
			str = "%" + str + "%";
		}
		return str;
	}
	...
	/**
	 * BigDecimal格式化2位小数 <br>
	 * 
	 * @param big
	 *            字符
	 * @return 处理后的值
	 */
	public static String bigDecimalFormat(BigDecimal big) {
		String str = "";
		if (big != null && !big.equals(new BigDecimal(0))) {
	 ...
	 * @return
	 */
	public static String getStrNull2Empty(String str) {
		if (StringUtil.isEmpty(str)) {
			return "";
		}
		return str.trim();
	 ...
	 * @return
	 */
	public static String objectToString(Object i) {
		if (null == i) {
			return null;
		}
		return i.toString();
	 ...
	 * 
	 * @param str
	 * @return true表示是浮点类型，false表示不是浮点类型
	 */
	public static boolean isBigDecimal(String str) {
		try {
			String price = str.toString();
	 ...
	 */
	public static boolean isRightPointNum(String str, int num) {
		try {
			String price = str.toString();
			new BigDecimal(price);
			if (price.indexOf(".") > -1 && price.split("\\.")[1].length() > num) {
				return false;
			}
		} catch (Exception e) {
			return false;
	 ...
	 * @return true表示正确，false表示不正确
	 */
	public static boolean isLeftPointNum(String str, int num) {
		try {
			String price = str.toString();
			new BigDecimal(price);
			if (price.split("\\.")[0].length() > num) {
				return false;
			}
		} catch (Exception e) {
			return false;
	 ...
	 * @return
	 */
	public static String getSum4StrNum(String num1, String num2) {
	 ...
	 * @return
	 */
	public static String getSubtract4StrNum(String num1, String num2) {
		if (isEmpty(num1) || isEmpty(num2)) {
			return null;
		}
		if (!isInteger(num1) || !isInteger(num2)) {
	 ...
	 * 参数拆分多个数组
	 * <p>
	 * 长度超过maxLength字符时拆分为多个数组
	 * </p>
	 * 
	 * @param params
	 *            参数 例："AAAAA,BBBBB,CCCCC,DDDDD,EEEEE,FFFFF"
	 ...
	 * @return
	 */
	public static String[] getParamArray(String params, String separatorChars, int maxLength) {
		if (StringUtil.isEmpty(params) || StringUtil.isEmpty(separatorChars)) {
			return null;
		}
		String[] array = null;
		if (params.length() > maxLength) {
			// 单个参数
			String item = StringUtil.split(params, separatorChars)[0];
			int itemLen = item.length() + 1;
			// 拆分数组后，每个数组中单个参数的数量
			int count = maxLength / itemLen;
			int totalCount = params.length();
			// 每个数组中实际字符长度
			int actCount = count * itemLen;
			// 数组size
			int arraySize = params.length() / actCount;
			if (params.length() % actCount > 0) {
				arraySize += 1;
			}
			array = new String[arraySize];
			for (int i = 0; i < arraySize; i++) {
				if (i == arraySize - 1) {
					array[i] = params.substring(i * count * itemLen, totalCount);
				} else {
					String str = params.substring(i * count * itemLen, (i + 1) * count * itemLen);
					str = subEndsWith(str, separatorChars);
					array[i] = str;
				}
			}
		} else {
			array = new String[] { params };
		}
		return array;
	}
